<!-- officemd: kind=pptx profile=compact first_row_as_header=true formulas=true headers_footers=true properties=false -->

## Slide 1 - Quarterly Review

Revenue is up 12% quarter over quarter.
| Region | Revenue | Growth |
| --- | --- | --- |
| EMEA | $1.2M | 12% |
| NA | $0.98M | 8% |

### Notes
Speaker note: Call out that EMEA outperformed expectations.

### Comments
[^c1|Presenter]: Add one slide on operating margin.

## Slide 2 - Roadmap

Next: launch SKU refresh and expand partner channels.
