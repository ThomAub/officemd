// officemd vendor: this crate is excluded from clippy in CI.  We keep a
// blanket allow so local `cargo clippy` runs against the whole workspace
// don't flood with upstream lints we don't own.
#![allow(clippy::all)]
...
// not vendored — officemd has its own bindings in `crates/officemd_python`.
...
pub use detector::{
    DetectionConfig, PdfType, PdfTypeResult, ScanStrategy, detect_pdf_type, detect_pdf_type_mem,
    detect_pdf_type_mem_with_config, detect_pdf_type_with_config,
};
pub use extractor::{extract_text, extract_text_with_positions, extract_text_with_positions_pages};
pub use markdown::{
    MarkdownOptions, to_markdown, to_markdown_from_items, to_markdown_from_items_with_rects,
};
...
// =========================================================================
// officemd vendor: wasm32 timer shim
// =========================================================================
//
// Upstream uses `std::time::Instant` directly.  That type is unavailable on
// `wasm32-unknown-unknown`, so we route every call through a small shim that
// falls back to `js_sys::Date::now()` in the browser.  All upstream call
// sites have been rewritten to use `ProcessTimer`/`start_timer`/`elapsed_ms`.
...
pub struct PdfOptions {
    ...
    /// officemd vendor extension: when `true`, attempt text extraction even
    /// for Scanned/ImageBased PDFs instead of returning `None` immediately.
    /// Extraction failure is treated as non-fatal (like Mixed PDFs).
...
impl PdfOptions {
    ...
    /// officemd vendor extension: force text extraction even for
    /// Scanned/ImageBased PDFs.  Useful when a PDF was misclassified or when
    /// the caller wants to attempt extraction regardless.
...
pub fn extract_tables_with_structure_cells_mem(
    ...
    use tables::structured::{
        StructuredCell, cell_px_to_page_pt, normalize_cell_bands, parse_structure, polygon_to_aabb,
    };
...
fn load_document_bytes(buf: &[u8]) -> Result<Document, lopdf::Error> {
    // officemd vendor: upstream calls
    // `Document::load_mem_with_options(buf, lopdf::LoadOptions::with_password(""))`,
    // which only exists on the firecrawl/lopdf fork.  We use the crates.io
    // `lopdf` 0.39 equivalent so officemd remains publishable to crates.io.
...
fn process_document(
    ...
    // Scanned / ImageBased → nothing to extract (unless caller forces it
    // via the officemd `force_extract` option, in which case we fall through
    // to extraction and treat any failure as non-fatal, like Mixed PDFs).
    ...
    // For Mixed PDFs (or any forced extraction), failure is non-fatal: we
    // return `None` for markdown rather than propagating the error.
    let force_failure_non_fatal = pdf_type == PdfType::Mixed
        || (options.force_extract && matches!(pdf_type, PdfType::Scanned | PdfType::ImageBased));
    let extracted = if force_failure_non_fatal {
        extracted.ok()
    } else {
        Some(extracted?)
    };
...
mod tests {
    ...
    fn tsr_text_fill_does_not_pull_neighboring_overlapping_rows() {
        use crate::pdf_inspector::tables::StructuredCell;
        use crate::pdf_inspector::tables::structured::normalize_cell_bands;
