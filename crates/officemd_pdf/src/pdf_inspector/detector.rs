...
pub fn detect_pdf_type_with_config<P: AsRef<Path>>(
    ...
    let (doc, page_count) = crate::pdf_inspector::load_document_from_path(&path)?;
...
pub fn detect_pdf_type_mem_with_config(
    ...
    let (doc, page_count) = crate::pdf_inspector::load_document_from_mem(buffer)?;
...
fn identity_h_font_has_fallback(font_dict: &lopdf::Dictionary, doc: &Document) -> bool {
    ...
    if crate::pdf_inspector::tounicode::cid_values_look_like_unicode(cid_font_dict) {
        return true;
    }
