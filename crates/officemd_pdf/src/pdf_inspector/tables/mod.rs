...
pub use detect_rects::{RectHintRegion, detect_tables_from_rects};
pub use detect_struct::detect_tables_from_struct_tree;
pub use format::table_to_markdown;
pub use structured::{StructuredCell, cells_to_markdown};
...
pub(crate) fn try_build_table_from_columns(items: &[TextItem], page: u32) -> Option<Table> {
    use crate::pdf_inspector::extractor::{
        ColumnRegion, detect_columns, group_into_lines_with_thresholds, is_newspaper_layout,
    };
    ...
    let per_column_lines: Vec<Vec<crate::pdf_inspector::types::TextLine>> = col_buckets
        .iter()
        .map(|bucket| {
...
mod tests {
    use super::*;
    use crate::pdf_inspector::types::{ItemType, TextItem};
