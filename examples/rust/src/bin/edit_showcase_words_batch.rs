use officemd_core::{
    DocxPatch, DocxTextScope, PatchedDocument, PptxPatch, PptxTextScope, ScopedDocxReplace,
...
fn write_batch_results(
    ...
    format_name: &str,
    extension: &str,
    results: &[PatchedDocument],
) {
    for (idx, item) in results.iter().enumerate() {
        let path = out_dir.join(format!("showcase_batch_{idx}.{extension}"));
        fs::write(&path, &item.content).expect("write batch result");
        println!(
            "{}",
...
fn main() {
    ...
    let docx_patch = build_docx_patch();
    let docx_results =
    ...
    let pptx_patch = build_pptx_patch();
    let pptx_results =
    ...
    let xlsx_patch = build_xlsx_patch();
    let xlsx_results =
        patch_xlsx_batch_with_report(vec![xlsx_bytes.clone(), xlsx_bytes], &xlsx_patch, Some(2))
            .expect("patch xlsx batch");

    write_batch_results(&out_dir, "docx", "docx", &docx_results);
    write_batch_results(&out_dir, "pptx", "pptx", &pptx_results);
    write_batch_results(&out_dir, "xlsx", "xlsx", &xlsx_results);
}
